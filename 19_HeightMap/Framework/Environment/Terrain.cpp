...
void Terrain::CreateVertexData()
{
	...
	for (UINT z = 0; z < height; z++) //텍스처 정점수를 사용하게 되면 한칸이 모자른다. 한칸 줄여버리고 한다.(이전거랑 차이가 없다.)
	{
		for (UINT x = 0; x < width; x++)
		{
			...
			//x와 z값은 그대로 놔두고 height만 가져다가 y에쓸 것이다.
			vertices[index].Position.x = (float)x;
			//rgba가 다같으니 그냥 r을 불러오겠다. 255로 늘리고 너무크니까 10 나눴다.
